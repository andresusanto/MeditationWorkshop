...
$("#menu2").mouseleave(
	...
	<tr>
		<td colspan="12" rowspan="5">
			<!-- <img src="<?php echo $sitepath; ?>images/content_23.png" width="583" height="340" alt="">-->
  ...
  <div id="panel2">
	...
	if ($artikel) { 
		...
		<font size="+1"><?php echo $artikel[$i][3]; ?></font><small><br/>
		<i>Ditulis oleh <?php echo getUserbyID($artikel[$i][1]); ?> pada <?php echo date("d M Y H:i", $artikel[$i][2]); ?></i></small><br/>
		<?php if (stripos($artikel[$i][4],"%READMORE%") === FALSE) { 
				echo $artikel[$i][4] . '<hr/>';
				}else{
			$towrite = explode("%READMORE%", $artikel[$i][4]);
			echo strip_tags($towrite[0]);
			echo '<br/><a href="' . $sitepath .'index.php/read/'.$artikel[$i][0].'">Read more...</a><hr/>';
			}?>
	<?php } if ($total = db_num("SELECT * FROM artikel") > 5) { 
				is_numeric($request[1]) ? $p = $request[1] : $p = 1;
				$jumPage = ceil($total / 5); 
				$start = ($p - 1) * 10;
				...
				if ($p == 1) {
					if ($jumPage <= 1){
						...
						$PageNumber = '<font size="+1">1</font> ';
						for ($i=2; $i<=$jumPage; $i++){
							$PageNumber = $PageNumber . '<a href="'.$sitepath . 'index.php/main/' .$i.'">'.$i.'</a> ';
							if ($i == 7) break;
						}
						$PageNumber = $PageNumber. '<a href="'.$sitepath . 'index.php/main/2">Next&gt;</a> <a href="'.$sitepath . 'index.php/main/' . $jumPage.'">Last&gt;&gt;</a>';
							
					}
				}elseif ($p == $jumPage) {
					$PageNumber = '<a href="'.$sitepath . 'index.php/main/1">&lt;&lt;First</a> <a href="'.$sitepath . 'index.php/main/'. ($jumPage - 1) .'">&lt;Before</a> ';
					if ($jumPage > 6) {
						$start = $jumPage - 6;
					}else{
						$start = 1;
					}
					for ($i=$start; $i<$jumPage; $i++){
						$PageNumber = $PageNumber . '<a href="'.$sitepath . 'index.php/main/'.$i.'">'.$i.'</a> ';
					}
					$PageNumber = $PageNumber. '<font size="+1">' . $jumPage . '</font>';
				}else{
					if ($p > 3) {
						$start = $p - 3;
					}else{
					...
					if ($jumPage - $start > 6) {
						...
						$finish = $jumPage;
						if ($jumPage > 7) $start = $finish - 6;
					}
					if ($finish - $start < $jumPage && $jumPage < 8) $start = 1;
					$PageNumber = '<a href="'.$sitepath . 'index.php/main/1">&lt;&lt;First</a> <a href="'.$sitepath . 'index.php/main/'. ($p - 1) .'">&lt;Before</a> ';
					for ($i=$start; $i<$p; $i++){
						$PageNumber = $PageNumber . '<a href="'.$sitepath . 'index.php/main/'.$i.'">'.$i.'</a> ';
					}
					$PageNumber = $PageNumber . '<font size="+1">' . $p . '</font> ';
					for ($i=$p + 1; $i<=$finish; $i++){
						$PageNumber = $PageNumber . '<a href="'.$sitepath . 'index.php/main/'.$i.'">'.$i.'</a> ';
					}
					$PageNumber = $PageNumber. '<a href="'.$sitepath . 'index.php/main/' . ($p + 1) . '">Next&gt;</a> <a href="'.$sitepath . 'index.php/main/'. $jumPage.'">Last&gt;&gt;</a>';
				}
				?>
